...
do
echo "${id}"
featureCounts -T 40  -p -B -C -s 2 -t exon -g gene_id -F GTF -a /media/wangjiachen/disk6/test/scripts/genome/5_Bd/Brachypodium_distachyon.Brachypodium_distachyon_v3.0.55.gtf -o ${id}.count ${id}.bam
echo "count done"
